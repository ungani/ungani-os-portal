-- Adds ungani_customer_invoices to get_my_ungani_recently_deleted_v2()'s
-- table list (this is the read side that my-recently-deleted.html
-- actually queries - it live-scans each table by deleted_at, it does NOT
-- read from ungani_recently_deleted, so the earlier soft_delete_ungani_record
-- fix alone wasn't enough to make deleted invoices visible here).
--
-- Every other table's title logic (a single-column priority coalesce) is
-- untouched. Invoices get one deliberate exception: a combined
-- "invoice number · customer name" title, since this table is the only
-- one where a single column isn't meaningful enough on its own - that's
-- the one non-mechanical addition, confirmed via pg_get_functiondef
-- before writing this, everything else is byte-for-byte identical to the
...
begin
  ...
  loop
    ...
    -- Invoices need both the invoice number and the customer name to be
    -- meaningful in this list (every other table here only ever shows one
    -- column), so this is the one deliberate exception to the generic
    -- single-column-priority title above.
