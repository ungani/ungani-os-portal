...
(function () {
    ...
    return (
      '<div class="ungani-print-brand" style="display:flex;gap:14px;align-items:center;">' +
        '<img src="' + escapeHtml(getLogoUrl(tenant)) + '" alt="' + escapeHtml(getDisplayName(tenant)) + ' logo" ' +
          'style="width:' + logoSize + 'px;height:' + logoSize + 'px;object-fit:contain;background:#FFFFFF;border-radius:18px;padding:7px;" ' +
          'onerror="this.onerror=null;this.src=\'ungani-logo.png\';" />' +
        '<div>' +
          '<h1 style="margin:0 0 5px;font-size:26px;letter-spacing:0.3px;">' + escapeHtml(getDisplayName(tenant)) + '</h1>' +
          (contactHtml ? '<p style="margin:0;color:#B8C3D6;line-height:1.5;font-size:13px;">' + contactHtml + '</p>' : '') +
        '</div>' +
      '</div>'
    );
  }
  ...
  // UNGANI OS stays the platform brand per explicit requirement.
  function renderPoweredByFooter() {
    return '<div class="ungani-powered-by" style="text-align:center;font-size:11px;opacity:0.6;margin-top:14px;">Powered by UNGANI OS</div>';
  }
