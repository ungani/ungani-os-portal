(function () {
  const COLORS = {
    ...
    softGreen: "rgba(22, 163, 74, 0.15)",
    softGold: "rgba(212, 166, 58, 0.18)",
    softBlue: "rgba(37, 99, 235, 0.14)",
    softRed: "rgba(220, 38, 38, 0.14)"
  };
  ...
  function loadChartJs() {
    ...
    chartJsPromise = new Promise(function (resolve, reject) {
      ...
      script.onerror = function () {
        reject(new Error("Chart.js could not load."));
      };
  ...
  function injectChartStyles() {
    if (document.getElementById("ungani-analytics-styles")) {
      return;
    }
    ...
    style.textContent = `
      ...
      .ungani-chart-canvas-wrap {
        position: relative;
        width: 100%;
        height: 270px;
      }

      .ungani-chart-canvas-wrap.small {
        height: 230px;
      }
      ...
      html[data-ungani-theme="dark"] .ungani-project-track {
  ...
  function compactMoney(value) {
    ...
    if (absolute >= 1000000000) {
      return "Ksh " + sign + trimDecimal(absolute / 1000000000) + "B";
    }

    if (absolute >= 1000000) {
      return "Ksh " + sign + trimDecimal(absolute / 1000000) + "M";
    }

    if (absolute >= 1000) {
      return "Ksh " + sign + trimDecimal(absolute / 1000) + "K";
    }
  ...
  function normalizeExpenseCategory(category) {
    ...
    if (lower.includes("maintenance") || lower.includes("repair")) return "Property Maintenance";
    if (lower.includes("staff") || lower.includes("wage")) return "Staff Wages";
    if (lower.includes("transport") || lower.includes("site visit")) return "Site Visit / Transport Cost";
  ...
  function getExpenseCategoryColor(category) {
    ...
    if (EXPENSE_CATEGORY_COLORS[lower]) {
      return EXPENSE_CATEGORY_COLORS[lower];
    }
    ...
    if (lower.includes("maintenance") || lower.includes("repair")) return EXPENSE_CATEGORY_COLORS["property maintenance"];
    if (lower.includes("staff") || lower.includes("wage")) return EXPENSE_CATEGORY_COLORS["staff wages"];
    if (lower.includes("transport") || lower.includes("site visit")) return EXPENSE_CATEGORY_COLORS["site visit / transport cost"];
  ...
  function calculateExpenseBreakdown(transactions) {
    ...
    return Object.keys(map).map(function (key) {
      return {
        ...
        color: getExpenseCategoryColor(key)
      };
    }).sort(function (a, b) {
  ...
  function calculatePropertySummary(items) {
    ...
    (items || []).forEach(function (row) {
      ...
      if (status.includes("sold") || status.includes("rented") || status.includes("closed")) closed++;
      if (status.includes("reserved")) reserved++;
      if (status.includes("negotiation")) negotiation++;
    });
  ...
  function calculatePeopleSummary(people) {
    let agents = 0;
    let leads = 0;
    let staff = 0;
    let customers = 0;
    ...
    (people || []).forEach(function (row) {
      ...
      if (type === "lead" || type === "buyer" || type === "renter" || type === "tenant") leads++;
      if (type === "staff" || type === "manager") staff++;
      if (type === "customer" || type === "client") customers++;
    });
  ...
  function calculateProjects(items) {
    ...
    return Object.keys(map).map(function (key) {
      ...
      if (item.progressCount > 0) {
        item.progress = Math.round(item.progressTotal / item.progressCount);
      } else if (item.totalUnits > 0) {
        item.progress = Math.round((item.unitsSold / item.totalUnits) * 100);
      } else {
        item.progress = 0;
      }
  ...
  function calculateAgentPerformance(people, isRealEstate) {
    const agents = (people || []).filter(function (row) {
      const type = String(safeGet(row, ["person_type"], "")).toLowerCase();
      ...
      return type === "staff" || type === "manager" || type === "agent" || type === "sales";
    });
  ...
  function calculateLeadPipeline(people, records, isRealEstate) {
    const map = {};

    (people || []).forEach(function (row) {
      const type = String(safeGet(row, ["person_type"], "")).toLowerCase();

      if (isRealEstate && type !== "lead" && type !== "buyer" && type !== "renter" && type !== "tenant") return;
    ...
    (records || []).forEach(function (row) {
      ...
      if (isRealEstate && !recordType.includes("lead") && !recordType.includes("viewing") && !recordType.includes("inquiry")) return;

      const stage = safeGet(row, ["lead_status", "deal_status", "status"], "Record");
      map[stage] = (map[stage] || 0) + 1;
    });
  ...
  function chartTextColor() {
    const isDark = document.documentElement.dataset.unganiTheme === "dark";
    return isDark ? "#E5E7EB" : "#334155";
  }

  function chartGridColor() {
    const isDark = document.documentElement.dataset.unganiTheme === "dark";
    return isDark ? "rgba(255,255,255,0.12)" : "rgba(6,28,61,0.10)";
  }
    ...
    return createChart(canvasId, {
      ...
      data: {
        ...
        datasets: [
          {
            label: "Income",
            data: rows.map(function (row) { return Number(row.income || 0); }),
            backgroundColor: COLORS.green,
            borderRadius: 10
          },
          {
            label: "Expenses",
            data: rows.map(function (row) { return Number(row.expenses || 0); }),
            backgroundColor: COLORS.gold,
            borderRadius: 10
          },
          {
            label: "Balance",
            data: rows.map(function (row) { return Number(row.balance || 0); }),
            type: "line",
